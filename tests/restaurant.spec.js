...
describe("10 - Implemente a função `createMenu`, bem como seus casos de teste", () => {
  it("Verifica se a função `createMenu` tem o comportamento esperado", () => {
    // TESTE 1: Verifique se função `createMenu()` retorna um objeto que possui a chave `fetchMenu`,
    // a qual tem como valor uma função.
    // ```
    // const objetoRetornado = createMenu(); // Retorno: { fetchMenu: () => {}, ... }
    // ```
    const objetoRetornado = createMenu({
      food: { coxinha: 1, sopa: 0.8 },
      drink: { agua: 0.6, cha: 0.7 },
    });
    expect(typeof objetoRetornado.fetchMenu).toBe("function");
    // TESTE 2: Verifique se 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são
    // somente `food` e `drink`,
    ...
    objetoRetornado.order("agua");
    objetoRetornado.order("coxinha");
    expect(objetoRetornado.pay()).toBeCloseTo(2.86);
